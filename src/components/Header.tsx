...
import { HiOutlineSignal, HiOutlineGlobeAlt, HiOutlineMoon, HiOutlineSun } from 'react-icons/hi2';
      ...
      <div className="flex items-center gap-4 px-6 py-3">
        ...
        <div className="flex items-center gap-2 flex-1 max-w-md">
          ...
          <input
            ...
            className="flex-1 rounded-lg border border-slate-300 dark:border-slate-600/50 bg-slate-50 dark:bg-slate-800/60 px-3 py-1.5 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 disabled:opacity-50 transition-colors"
          />
        </div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={`h-2.5 w-2.5 rounded-full ${
                connected ? 'bg-green-500 dark:bg-green-400 animate-pulse-glow' : 'bg-slate-400 dark:bg-slate-500'
              }`}
            />
            <span className="text-xs text-slate-500 dark:text-slate-400">{connected ? 'Online' : 'Offline'}</span>
          </div>
        </div>
      </div>
    </header>
